...
if isempty(C)
    if ~con.learn
        error('Connection values must be specified if not learning')
    end
    A0 = zeros(size(con.F));
...
elseif strcmpi(con.type, '3freq')
    mask = con.mask;
elseif con.n1 == con.n2 && con.nSourceClass == 2
