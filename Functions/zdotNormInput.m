...
%%
function [dzdt, maxInput] = zdotNormInput(M, nx, varargin)
...
normInput = n.normInput;    % normalized total amplitude value
maxInputPrev = n.maxInputPrev;  % previous smoothed max input
d = n.maxInputDecay;    % decay rate

xMax = max(abs(x)./n.f);    % current max, assuming log network

maxRatio = xMax/maxInputPrev + eps('single');
% d = tanh((abs(log10(maxRatio)))*3-3)/2*(1-d) + (1+d)/2;   % adaptive d: approaches 1--no smoothing--for large abs(maxRatio)

if maxRatio > 1
    d = tanh(log10(maxRatio)*3-3)/2*(1-d)+(1-d)/2 + d;      % adaptive d: approaches 1--no smoothing--for large maxRatio
end

maxInput = (1-d)*maxInputPrev + d*xMax + eps('single');    % smoothed max input amplitude
x = x/maxInput*normInput;   % normalized input

%% Write to base workspace (for testing)
% currentMax = evalin('base','currentMax');
% smoothMax = evalin('base','smoothMax');
% assignin('base','currentMax',[currentMax xMax])
% assignin('base','smoothMax',[smoothMax maxInput])
